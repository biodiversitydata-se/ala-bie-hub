...
function showSpeciesPage(traitsTabSet) {
    ...
    addAlerts();
    loadBhl();
    // SBDI: loadTrove crashes if it's not configured
...
function loadOverviewImages(){
    ...
    if (SHOW_CONF.preferredImageId) {
        ...
        $.getJSON(prefUrl, function(data){
            ...
            console.error('Error loading overview image: ' + textStatus + ', ' + error);
    ...
    $.getJSON(url, function(data){
        ...
        console.error('Error loading overview images: ' + textStatus + ', ' + error);
    }).always(function() {
        $('#gallerySpinner').hide();
    });
}
